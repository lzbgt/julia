...
mutable struct Backtrack{T, S<:AbstractVector, O, C}
    target::T
    states::S
    out::O
    cache::C
...
Backtrack(target, len::Int, states, out) =
    Backtrack(target, states, out, spzeros(Int, len), 1, len, 1)

function Backtrack(target::AbstractString, state1)
    len = length(target)
    # Setting max_pos to 1 means that backtrack[1] == 0
    states = Vector{typeof(state1)}(len)
    states[1] = state1
    return Backtrack(target, len, states, IOBuffer())
end

#function Backtrack(target::String, state1::Int)
#    t = Vector{UInt8}(target)  # convert String to a utf8-byte-iterator
#    len = length(t)
#    return Backtrack(t, len, OneTo(len), StringVector(0))
#end

function prep_backtrack_cache(bt::Backtrack, index::Int)
    # grow cache up to index
    for i in (bt.max_pos + 1):index
        b = bt.cache[i - 1] + 1
        # Requires that i > b. We can avoid this check since we start max_pos at 1.
        ci = next(bt.target, bt.states[i])[1]
        cb = next(bt.target, bt.states[b])[1]
        if ci == cb
            bt.cache[i] = b
        end
    end
...
@inline function record(bt::Backtrack, c)
    ...
    pos = bt.pos
    while true
        c1, pos1 = next(bt.target, bt.states[pos])
        if c == c1
            pos += 1
            isa(bt.states, Vector) && (bt.states[pos] = pos1)
            break
        end
        pos == 1 && break
        pos -= 1
        prep_backtrack_cache(bt, pos)
        pos = bt.cache[pos]
    end
    bt.pos = pos
    return pos == bt.len
...
take!(bt::Backtrack) = isa(bt.out, IO) ? take!(bt.out) : bt.out
...
function readuntil(io::IO, target::AbstractString)
    ...
    backtrack = Backtrack(target, i)
    while !eof(io)
        c = read(io, Char)
        record(backtrack, c) && break
    end
    return String(take!(backtrack))
end
